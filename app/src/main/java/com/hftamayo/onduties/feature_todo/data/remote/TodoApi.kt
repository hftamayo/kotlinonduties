package com.hftamayo.onduties.feature_todo.data.remote

import com.hftamayo.onduties.feature_todo.data.remote.dto.RemoteTodoItem
...
interface TodoApi {
    ...
    //@POST("todo.json")
    //suspend fun addTodo(@Body url: String, @Body updateTodo: RemoteTodoItem): Response<Unit>

    @PUT
    suspend fun addTodo(@Url url: String, @Body updatedTodo: RemoteTodoItem): Response<Unit>

    @DELETE("todo/{id}.json")
    suspend fun deleteTodoById(@Path("id") id: Int?): Response<Unit>
